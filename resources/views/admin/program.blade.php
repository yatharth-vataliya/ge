		...
		<div class="col-md-4 border border-primary p-2 p-md-2">
			<form method="POST" action="{{ route('insert_program') }}" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="program_name" id="program_name" placeholder="Program Name" value="{{ old('program_name') }}">
				</div>
				<div class="form-group">
			...
			<table class="table table-striped table-hover table-responsive">
				...
				<tbody>
					...
					<tr id="row{{ $program->id }}">
						...
						<td><a href="{{ route('edit_program',[ 'program_id' => $program->id ]) }}" class="btn btn-warning">Edit</a></td>
						<td><button type="button" class="btn btn-danger" onclick="delete_program({{ $program->id }});">Delete</button></td>
					</tr>
					@endforeach
				</tbody>
			</table>
