                ...
                <form method="POST" action="{{ route('insert_department') }}" enctype="multipart/form-data">
                        ...
                        <select name="institute_id" id="institute" class="custom-select">
                            ...
                            @foreach ($institutes as $institute)
                                @if (old('institute_id') == $institute->id)
                                    <option value="{{ old('institute_id') }}" selected>{{ $institute->institute_name }}
                                    </option>
                                @endif
                    ...
                    <div class="form-inline form-group">
                        <div class="form-check form-check-inline">
                            TYPE :- &nbsp;
                            <input type="radio" name="type" id="PG" value="PG" class="form-check-input"
                                {{ old('type') == 'PG' ? 'checked' : '' }}>
                            <label for="PG" class="form-check-label">PG</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input type="radio" name="type" class="form-check-input" value="UG" id="UG"
                                {{ old('type') == 'UG' ? 'checked' : '' }}>
                    ...
                    <tbody>
                        @foreach ($departments as $department)
                            <tr id="row{{ $department->id }}">
                                ...
                                <td><a href="{{ route('edit_department', ['department_id' => $department->id]) }}"
                                        class="btn btn-warning">Edit</a></td>
                                <td><button type="button" class="btn btn-danger"
                                        onclick="delete_department({{ $department->id }});">Delete</button></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
